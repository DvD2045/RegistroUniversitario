    ...
    public ResponseEntity<?> registerUser(@Valid @RequestBody SignupRequest signUpRequest) {
        ...
        if (strRoles == null) {
            ...
            strRoles.forEach(role -> {
                switch (role) {
                    case "admin" -> roles.add(rolRepository.findByNombre(Rol.NombreRol.ROL_ADMIN)
                            .orElseThrow(() -> new RuntimeException("Error: Rol no encontrado.")));
                    case "docente" -> roles.add(rolRepository.findByNombre(Rol.NombreRol.ROL_DOCENTE)
                            .orElseThrow(() -> new RuntimeException("Error: Rol no encontrado.")));
                    default -> roles.add(rolRepository.findByNombre(Rol.NombreRol.ROL_ESTUDIANTE)
                            .orElseThrow(() -> new RuntimeException("Error: Rol no encontrado.")));
                }
            });
        }
